...
{
    Console.WriteLine("Missing config.ini file");
...
string importCodePath = configuration["universal:importcode"];
ulong currentHash = 0;
bool IsInteractive;

if (!Directory.Exists(importCodePath))
{
    Directory.CreateDirectory(importCodePath);
}
string[] dirFiles = Directory.GetFiles(importCodePath);
...
{
    currentHash = FileHasher.ComputeFileHash(backupDataPath);
}
...
{
    Console.WriteLine("Game Data Hash Mismatch, This happens because loader is outdated or the data.win is modified. Try deleting backup.win and verify the integrity of game.");
    Console.WriteLine("Press any key to close...");
    Console.ReadKey();
    Environment.Exit(0);
}
else
{
    ...
    Console.WriteLine("Backup of the data has been created");
}
Console.WriteLine($"Loaded game {Data.GeneralInfo.Name.Content}");

SyncBinding("Strings, Code, CodeLocals, Scripts, GlobalInitScripts, GameObjects, Functions, Variables", true);
await Task.Run(() => {
    foreach (string file in dirFiles)
    {
        ImportGMLFile(file, true, false, false);
    }
});
...
Console.WriteLine("GML Loader by Senjay, Have fun");
Thread.Sleep(5000);
Environment.Exit(0);
#endregion

#region Method
// I'm too lazy to optimize the scripts so there's a lot of obsolete scripts in here
...
/*
//Hash checker
string filePath = "data.win";
ulong fileHash = FileHasher.ComputeFileHash(filePath);
Console.WriteLine($"Hash Value: {fileHash}");
*/